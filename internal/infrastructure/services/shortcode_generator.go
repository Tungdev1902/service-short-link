...
// NewUniqueCodeService creates a new unique code service
func NewUniqueCodeService() domain.UniqueCodeService {
	return &uniqueCodeService{}
}

// GenerateUniqueCode generates a unique short code with collision detection
func (s *uniqueCodeService) GenerateUniqueCode(generator domain.ShortCodeGenerator, repo domain.LinkRepository, length int) (string, error) {
	batchSize := 5
	codes := make([]string, batchSize)

	for attempt := 0; attempt < maxRetries; attempt++ {
		for i := 0; i < batchSize; i++ {
			codes[i] = generator.GenerateWithLength(length + attempt)
		}

		availableCode := s.findAvailableCodeInBatch(repo, codes)
		if availableCode != "" {
			return availableCode, nil
		}
...
// findAvailableCodeInBatch checks which codes in the batch are available
func (s *uniqueCodeService) findAvailableCodeInBatch(repo domain.LinkRepository, codes []string) string {
	for _, code := range codes {
		exists, err := repo.ExistsByShortCode(code)
		if err != nil {
			logger.ErrorWithCockroachSimple(err, "UniqueCodeService.findAvailableCodeInBatch: failed to check code existence", "code="+code, "error_type=database_query_failed")
			continue
		}
		if !exists {
			return code
